...
This function maps consequences (CSQ) of a given list of variants to
those in an annotated set based on facilities such as variant effect
predictor (VEP). In the case of protein-altering variants (PAVs), many
consequences can be involved. The procedure also takes into account
linkage disequilibrium (LD) in flanking regions.
...
-   ldops:
    ...
    Arguments for `ieugwasr::ld_matrix_local()`, typically a list with
    `bfile` and `plink` paths.
...
-   pop:
    ...
    `ieugwasr::ld_matrix()` (default is "EUR").
...
A data.frame with an indicator showing whether the consequences are
present or absent.
