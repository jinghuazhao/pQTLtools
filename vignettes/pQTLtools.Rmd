...
INF1_merge <- merged[c("Chrom","Start","End","prot","MarkerName")]
...
M <- 1e+6
INF1_merge <- merged %>%
              dplyr::mutate(chr=CHR,start=if_else(POS-M<0,1,POS-M),end=POS+M) %>%
...
coloc <- function(gwas_stats_hg38,ensGene,region38)
{
  ...
  fp <- file.path(system.file(package="pQTLtools"),"eQTL-Catalogue",
                  "tabix_ftp_paths_gtex.tsv")
  imported_tabix_paths <- within(read.delim(fp, stringsAsFactors = FALSE) %>%
                          dplyr::as_tibble(),
          {
...
f <- file.path(find.package("pQTLtools"),"tests",with(sentinel,paste0(prot,"-",gsub(":","_",SNP),".rda")))
coloc_df_imported <- coloc(gwas_stats_hg38,ensGene,region38)
save(gwas_stats_hg38,sentinel,coloc_df_imported,file=f)
```

We load the data obtained from above,

```{r coloc, fig.cap="Association plot", fig.height=7, fig.width=8}
load(file.path(find.package("pQTLtools"),"tests","OPG-chr8_120081031_C_T.rda"))
