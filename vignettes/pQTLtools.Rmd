...
```{r mht2d, fig.cap="2d pQTL plot", fig.height=8}
INF <- Sys.getenv("INF")
d <- read.csv(file.path(INF,"work","INF1.merge.cis.vs.trans"),as.is=TRUE)
t2d <- pqtl2d(d)
```
whose result can be also viewed in a plotly style,
```{r plotly, fig.cap="3d pQTL plot", fig.height=8, fig.width=8, plotly=TRUE}
d <- read.csv(file.path(INF,"work","INF1.merge.cis.vs.trans"), as.is=TRUE)
...
```{r mr, fig.cap="MR with cis, trans and cis+trans variants of IL-12B"}
library(dplyr)
...
d3 <- read.delim(file.path(INF,"mr","efo-result.txt")) %>%
      ...
      left_join(efo) %>%
      arrange(trait)
knitr::kable(head(d3,29),caption="Table 6. MR with IL-12B variants")
