...
```{r pqtl2dplot, fig.cap="2d pQTL plot", fig.height=8}
INF <- Sys.getenv("INF")
d <- read.csv(file.path(INF,"work","INF1.merge.cis.vs.trans"),as.is=TRUE)
t2d <- pqtl2dplot(d)
```
whose result can be also viewed in a 2-d plotly style,
```{r pqtl2dplotly, fig.cap="2d pQTL plotly", fig.height=8, fig.width=8, plotly=TRUE}
fig2d <- pqtl2dplotly(d)
fig2d
# We can save the figure for a browser independently
htmlwidgets::saveWidget(fig2d,file="fig2d.html")
```
and 3-d counterpart
```{r pqtl3dplotly, fig.cap="3d pQTL plotly", fig.height=8, fig.width=8, plotly=TRUE}
fig3d <- pqtl3dplotly(d,zmax=300)
fig3d
# We can save the figure for a browser independently
...
for(i in 1:180)
{
  ...
  g <- getBM(attributes = attrs, filters="chromosomal_region", values=v, mart=mart)
...
d3 <- read.delim(file.path(INF,"mr","efo-result.txt")) %>%
      ...
      left_join(efo) %>%
      arrange(cistrans)
knitr::kable(head(d3[c("MRBASEID","trait","cistrans","nsnp","b","se","pval")],29),caption="Table 6. MR with IL-12B variants")
