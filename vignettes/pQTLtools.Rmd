...
pkgs <- c("GenomicRanges", "TwoSampleMR", "biomaRt", "coloc", "dplyr", "gap", "ggplot2", "httr",
          "ieugwasr", "karyoploteR", "circlize", "knitr", "plotly", "pQTLtools",
...
exposure <- format_data(read.csv(f))
caption4 <- "IL6R variant and diseases"
kable(exposure, caption=paste(caption4,"(instruments)"),digits=3)
f <- file.path(system.file(package="pQTLtools"),"tests","Out.csv")
outcome <- format_data(read.csv(f),type="outcome")
...
We carry to producing a forest plot.
...
exposure <- TwoSampleMR::format_data(within(d,{P=10^logP}), phenotype_col="prot", snp_col="rsid",
                        ...
                        samplesize_col="N")
clump <- exposure[sample(1:nrow(exposure),nrow(exposure)/10),] # TwoSampleMR::clump_data(exposure)
outcome <- TwoSampleMR::extract_outcome_data(snps=clump$SNP,outcomes=outcomes)
...
At time of running (12/4/2024), `clump_data` fails to run so 10% sample is used for illustrative purpose.
...
gap::mr_forestplot(tnfb,colgap.forest.left="0.05cm", fontsize=14,
              leftcols=c("studlab"), leftlabs=c("Outcome"),
