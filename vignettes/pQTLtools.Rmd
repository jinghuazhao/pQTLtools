...
The `merge_cvt` object is obtained for later use. A more recent implementation is the `qtlClassifier` function. For this example we have,
...
sumstats <- function(prot,chr,region37)
{
  ...
  gwas_stats_hg38 <- rtracklayer::liftOver(gwas_stats, chain) %>%
                     ...
                     dplyr::mutate(gsub("chr","",chromosome))
}

gtex_coloc <- function(gwas_stats_hg38,ensGene,region38)
{
  cat("GTEx_v8 imported eQTL datasets\n")
...
ss <- subset(pQTLdata::inf1,prot==sentinel[["prot"]])
...
f <- with(sentinel,paste0(prot,"-",gsub(":","_",SNP),".rda"))
gwas_stats_hg38 <- sumstats(sentinel[["prot"]],chr,ensRegion37)
coloc_df_imported <- gtex_coloc(gwas_stats_hg38,ensGene,ensRegion38)
...
kable(within(coloc_df_imported,{qtl_id <- gsub("GTEx_V8_","",qtl_id)}), caption=ktitle)
```
