...
An alternative specification is via JSON, e.g., `cluster.json`
...
```bash
snakemake -j 99 --cluster-config cluster.json --use-conda --latency-wait 50 --cluster "
          sbatch -t {cluster.time} --output {cluster.output}
                                   --error {cluster.error}
                                   --nodes {cluster.nodes}
