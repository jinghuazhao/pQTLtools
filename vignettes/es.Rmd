...
beta.x[1]
lm(score~gender+AFFX.MurIL2_at,data=as.data.frame(exampleSet))
```
...
invisible(sapply(f,function(x) {
                     ...
                     par(mfrow=c(3,1)); hist(d,main=fn); plot(d, ylab=fn); boxplot(d)
                   }
          )
)
```
...
The quantity has been shown to have a big impact on protein abundance and therefore pQTL detection as is shown below.
...
options(width=120)
knitr::kable(annot,row.names=FALSE)
```
