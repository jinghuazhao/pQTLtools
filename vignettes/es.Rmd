...
pkgs <- c("Biobase", "arrayQualityMetrics", "dplyr", "knitr", "mclust", "pQTLtools", "OUTRIDER", "rgl", "scatterplot3d")
...
This is also related to outlier detection, e.g.,

```{r ods, fig.cap="Most significant outlier in Kremer data", fig.height=7, fig.with=7}
...
OUTRIDER::plotQQ(ods, res[1, geneID])
head(res)
```
...
```{r mcpca, fig.cap="Three-group Clustering", fig.height=7, fig.with=7}
  pca <- prcomp(na.omit(t(exprs(exampleSet))), rank=10, scale=TRUE)
  ...
  with(mc, {
      ...
      rgl::plot3d(with(pca,x[,c(2,1,3)]),cex=1.2,col=cols[classification],type="h")
  })
```
