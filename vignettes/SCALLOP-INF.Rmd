...
do
   ...
   if [ ! -f ${prot}-${MarkerName}.pdf ] || [ ! -f ${prot}-${MarkerName}.RDS ]; then
     cd ${INF}/coloc
     R --no-save < coloc.R 2>&1 | tee ${prot}-${MarkerName}.log
#    ls *tbi | xargs -I {} bash -c "rm {}"
     cd -
   fi
done
```
and the corresponding coloc.R contains minor modification to the documentation example
...
coloc_a <- function(gwas_stats_hg38,ensGene,region38)
{
  ...
  hdr <- file.path(find.package("pQTLtools",lib.loc=.libPaths()),"eQTL-Catalogue","column_names.CEDAR")
...
coloc_b <- function(gwas_stats_hg38,ensGene,region38)
{
  ...
  hdr <- file.path(find.package("pQTLtools",lib.loc=.libPaths()),"eQTL-Catalogue","column_names.Alasoo")
  column_names <- names(read.delim(hdr))
